...
export default function CartList() {
  ...
  async function handleCheckout(cartItems: CartItem[]) {
    try {
      ...
      const isCartValid = cartValidation.items.every((item: InvalidItems) => {
      ...
      {invalidItems && invalidItems.length > 0 && (
        <Alert variant="destructive" className="mb-12">
          ...
          <AlertDescription>
            ...
            {invalidItems.length !== 1 && "s"} with issues in your cart. Please
            review and remove them before proceeding to checkout.
          </AlertDescription>
        </Alert>
      )}
      {cartItems.map((item, idx) => {
        const isInvalid = invalidItems?.find(
          (i) => i.productId === item.productId && i.variantId === item.id
        );
          ...
          >
            ...
            {isInvalid ? (
              <div className="text-sm text-destructive space-y-1">
                ...
                {!isInvalid.valid && <p>This variant is no longer available</p>}
              </div>
            ) : (
